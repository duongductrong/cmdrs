...
const DatePicker = forwardRef<DatePickerExposeRef, DatePickerProps>(
  (
    {
    ...
    const handleChangeDate: SelectSingleEventHandler = (selectedDate) => {
      if (date && selectedDate) {
        const newDate = setYear(
          setMonth(
            setDayInMonth(date, selectedDate.getDate()),
          ...
          {withTime ? (
            <div className="flex w-full flex-wrap items-center justify-center gap-2 border-t p-3">
              <select
                ...
                onChange={(val) =>
                  handleTimeUnitChange(val.target.value, "hours")
                }
                value={hours}
              >
                {Array(24)
              ...
              <select
                className="rounded-md border px-2 py-1 text-sm"
                onChange={(val) =>
                  handleTimeUnitChange(val.target.value, "minutes")
                }
                value={minutes}
              >
                {Array(60)
              ...
              <select
                className="rounded-md border px-2 py-1 text-sm"
                onChange={(val) =>
                  handleTimeUnitChange(val.target.value, "seconds")
                }
                value={seconds}
              >
                {Array(60)
